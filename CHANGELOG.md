...
### 2. 1 号小部件（透明播放器）背景透明度可调
- 现在固定 50% 半透明
- 增加一个 0-100% 滑块
- 设置入口：App 内 PreferencesFragment「小部件 → 透明播放器 → 背景透明度」

### 3. 桌面歌词锁屏显示
- 3.9.0.5 因 MIUI/HyperOS `FLAG_SHOW_WHEN_LOCKED` 需要系统单独授权不稳定而删除
- 未来方案：监测 `ACTION_SCREEN_ON` / `ACTION_USER_PRESENT`，在用户解锁后重新应用悬浮窗 flags
- 配套：设置里加「桌面歌词锁屏显示」开关 + 首次使用跳转系统设置授权引导页

### 4. 4×1 紧凑小部件
- 比 4×2 更小，只显示：专辑封面（40dp）+ 标题/歌词（单行）+ 上一首/播放/下一首 3 控件
- 适合桌面空间有限的用户
- 颜色配置沿用 1 号 widget 的「纯色 / 透明」选项
