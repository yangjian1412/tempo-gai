...
- **通知栏歌词字号**(设置 → 歌词 → 通知栏歌词 → 歌词字号,默认中)
  - 3 档可选,小号匹配原版,中号/大号按 25%/50% 比例递增:
    - **小**(默认前一档:12sp 上下行 / 14sp 当前行):2 行显示(当前 + 下一句,与原版完全一致)
    - **中**(15sp 上下行 / 18sp 当前行):3 行显示(上 + 当前 + 下,中号比小号大约 25%)
    - **大**(18sp 上下行 / 21sp 当前行):2 行显示(大号比小号大约 50%,行数回退到 2 行避免过高)
