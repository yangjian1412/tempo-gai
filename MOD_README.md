...
- **特性**：
  ...
  - 屏幕旋转时位置自动重新计算
  - 退出应用/暂停播放时自动隐藏
  - 与原版"通知栏歌词"是互斥关系（避免视觉冲突）

### 2. 通知栏歌词

- **位置**：设置 → 歌词 → 通知栏歌词
- **行为**：在系统的媒体通知卡片下方追加一行当前歌词
- **特性**：
  - 大/小通知模板都支持
  - 跟随播放进度刷新（约 1 秒一次）
...
- **桌面歌词权限请求崩溃**：原版 `requestOverlayPermission()` 调用的 Activity 在小米/三星等定制 ROM 上不存在，导致开启桌面歌词时 100% 崩溃。已改为使用 `Settings.ACTION_MANAGE_OVERLAY_PERMISSION` 通用 Intent，附带 `ACTION_APPLICATION_DETAILS_SETTINGS` 兜底。
...
- **切歌时偶发"原 artist - 原 title"字符串重复叠加**：当前实现依赖"第一次注入时把原值存到 extras"，如果中途切歌且 extras 被清空，可能导致原值被注入值覆盖。如遇此问题，关闭再开启"系统播放器歌词"开关可恢复。
- **桌面歌词位置在分屏模式下不持久**：进入/退出分屏后需要重新拖动一次。
- **桌面歌词和通知栏歌词互斥**：同时只能开一个（设置里有提示），避免视觉冲突。
